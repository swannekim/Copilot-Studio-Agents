...
Copilot Studio에서 만드는 모든 것은 **harness** 위에서 돌아갑니다 — 내 설계와 모델 사이에 놓인 런타임입니다. 언제 모델을 호출할지, 무엇을 보낼지, 돌아온 결과를 어떻게 해석할지, 어떤 도구를 호출할지를 결정합니다. 현재 Microsoft는 세 가지를 제공하며, harness가 무엇을 만들 수 있는지, 무엇으로부터 복구할 수 있는지, 요금이 어떻게 부과되는지를 결정합니다.
