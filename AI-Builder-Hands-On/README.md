...
```
- for text contents on ```/JobDetails```, copy and paste ```Active Job Roles.txt```, for contents on ```/EvalCriteria```copy and paste ```Evaluation Criteria.txt```
    - keep in mind that this process is providing the sample format of the input

<details>
<summary>Example Contents of ResumeSummary</summary>

- you can copy and paste the answer from any resume summary generated by the agent during test session.
